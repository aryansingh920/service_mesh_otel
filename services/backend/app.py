...
resource = Resource.create(
    {"service.name": "backend", "deployment.environment": "mesh-apps"})
...
async def home(request: Request):
    ...
    try:
        ...
        logger.error(f"DB call failed: {e}", extra={
                     "upstream": "db-simulator"})
        span.record_exception(e)
        return JSONResponse(status_code=500, content={"service": "backend", "error": str(e)})

    return {"service": "backend", "message": "I am the Backend API", "db": db_data}
